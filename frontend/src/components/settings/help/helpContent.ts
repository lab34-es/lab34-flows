...
`,
  },
  {
    ...
    keywords: ['memory', 'lastResult', 'variables', 'handlebars', 'between steps', 'reuse'],
    body: `
Applications can write to the flow **memory**, and any later step can read it
...
Which keys a method writes (and reads) is part of its documentation, through
the \`@memory\` JSDoc tag — the application page lists them per method.

Memory lives for one run of one flow: it starts empty every time you press
*Run*.
`,
  },
  {
