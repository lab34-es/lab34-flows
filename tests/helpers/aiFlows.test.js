...
const aiFlows = require('../../src/helpers/aiFlows');
...
describe('aiFlows.create', () => {
  beforeEach(() => ai.generate.mockReset());
  ...
  test('returns the generated document', async () => {
    ...
    expect(ai.generate.mock.calls[0][0].prompt).not.toContain('divide');
  });
  ...
  test('retries once, telling the model what was wrong', async () => {
    ...
    expect(ai.generate.mock.calls[1][0].prompt).toContain('no ```step blocks');
  });
...
describe('aiFlows.edit', () => {
  ...
  test('sends the current document and returns the rewritten one', async () => {
    ai.generate.mockResolvedValue({ text: DOCUMENT, provider: 'gemini', model: 'gemini-2.5-flash' });

    const result = await aiFlows.edit({ prompt: 'Add an assertion', content: DOCUMENT });

    expect(result.flow).toBe(DOCUMENT.trim());
    expect(ai.generate.mock.calls[0][0].prompt).toContain('Add an assertion');
    expect(ai.generate.mock.calls[0][0].prompt).toContain('# Sums');
  });
});
