...
let cloudToken: { id: string; promise: Promise<string> } | null = null;
...
const searchIssues = async (settings, { jql, fields, onPage, stopped }: {
  jql: string;
  fields: any;
  onPage: (issues: any, total?: any) => void | Promise<void>;
  stopped?: () => boolean;
}) => {
  let cursor: Record<string, any> = { jql, fields };
  let seen = 0;
