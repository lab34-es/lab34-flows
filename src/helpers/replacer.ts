...
const tsAgo = (amount, lapse) => {
  ...
  const _milliseconds = result.getMilliseconds().toString().padStart(3, '0');
 ...
 */
/**
 * Parses a barcode mask into the array form `barcode` works with.
 *
 * A mask is literal text with `[n]` placeholders, each standing for n random
 * digits: "3232_[4]_247" -> ['3232_', 4, '_247'].
 *
 * @param {string} mask - The mask to parse
 * @returns {Array<string|number>} Literal segments interleaved with digit counts
 */
const parseBarcodePattern = (mask) => {
  ...
  while ((match = pattern.exec(mask)) !== null) {
    if (match.index > lastIndex) {
      parts.push(mask.slice(lastIndex, match.index));
    }
  ...
  if (lastIndex < mask.length) {
    parts.push(mask.slice(lastIndex));
  }
...
export const any = (input, values) => {
  ...
  try {
    result = json(input, values);
  }
  catch (_e) {
    result = string(input, values);
  }
