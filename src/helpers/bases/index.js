...
module.exports.defaultDisplayName = defaultDisplayName;
      ...
      columnSize: (view.columnSize && typeof view.columnSize === 'object' && !Array.isArray(view.columnSize))
        ? Object.fromEntries(
          Object.entries(view.columnSize)
            .map(([id, size]) => [normalizeProperty(id), Number(size)])
...
module.exports.normalizeDocument = normalizeDocument;
...
module.exports.viewsFilePath = viewsFilePath;
...
const load = async () => {
  ...
  try {
    ...
    throw new Error(`Invalid ${VIEWS_FILE}: ${ex.message}`);
  }
...
const save = async (document) => {
  ...
  out.views = normalized.views.map(view => {
    const entry = { type: view.type, name: view.name };
...
const buildScope = (entry, formulas) => {
  ...
  const scope = { note: meta, file, flow, absolutePath };
...
module.exports.collectFlows = collectFlows;
...
module.exports.HEADLINE_PROPERTIES = HEADLINE_PROPERTIES;
module.exports.VIEWS_FILE = VIEWS_FILE;
module.exports.DEFAULT_VIEW = DEFAULT_VIEW;
